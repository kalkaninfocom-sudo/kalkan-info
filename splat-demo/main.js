...
async function main() {
    ...
    const url = new URL(
        params.get("url") || "/assets/splat/train.splat",
        location.origin,
    );
    ...
    const rowLength = 3 * 4 + 3 * 4 + 4 + 4;
    // Handle servers that omit Content-Length (e.g. chunked transfer): read full body at once.
    const _clen = parseInt(req.headers.get("content-length")) || 0;
    let reader = null;
    let splatData;
    if (_clen > 0) {
        reader = req.body.getReader();
        splatData = new Uint8Array(_clen);
    } else {
    ...
    if (!reader) bytesRead = splatData.length;
    while (reader) {
