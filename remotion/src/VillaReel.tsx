...
/**
 * VillaReel — "Haftanın Villası" villa tanıtım reel'i (RestoranReel ikizi, 💰 flagship slot — Cmt 20:00).
 * Canlı site CAPTURE ETMEZ → çerez/install banner bug'ı YOK. İçerik data/villalar.json'dan
 * (build-villa-reel.mjs seçer) props ile gelir. 1080x1920, 30fps, 900 frame (30sn).
...
export const villaReelSchema = z.object({
  kicker: z.string().optional(),          // "KALKAN'DA KONAKLA"
  name: z.string(),
  category: z.string().optional(),        // "Premium"
  capacity: z.string().optional(),        // "8 kişi"
  bedrooms: z.string().optional(),        // "4 yatak odası"
  pool: z.string().optional(),            // "Özel ısıtmalı havuz"
  seaView: z.string().optional(),         // "Deniz manzaralı"
  location: z.string().optional(),        // kısa (mahalle/bölge)
...
export type VillaReelProps = z.infer<typeof villaReelSchema>;

export const defaultVillaReelProps: VillaReelProps = {
  kicker: "KALKAN'DA KONAKLA",
  name: 'Villa Poyraz',
  category: 'Premium',
  capacity: '8 kişi',
  bedrooms: '4 yatak odası',
  pool: 'Özel ısıtmalı havuz',
  seaView: 'Deniz manzaralı',
  location: 'Kalamar, Kalkan',
  phone: '+90 530 665 07 94',
  tagline: 'Kalamar sırtlarında, özel ısıtmalı havuzu ve seyir terasıyla lüks bir kaçış.',
  photos: [],
  cta: 'kalkaninfo.com/villalar',
};
...
const Intro: React.FC<VillaReelProps> = (p) => {
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', opacity: fade, padding: 80 }}>
      <div style={{ fontFamily: SANS, fontSize: 28, letterSpacing: 7, color: C.gold, fontWeight: 700, opacity: o, transform: `translateY(${(1 - o) * -20}px)` }}>
        {(p.kicker || "KALKAN'DA KONAKLA").toUpperCase()}
      </div>
      ...
      <div style={{ fontFamily: SANS, fontSize: 30, color: C.muted, fontWeight: 500, opacity: o2, letterSpacing: 2 }}>
        kalkaninfo.com · Konaklama Rehberi
      </div>
    </AbsoluteFill>
  );
};

// ── Sahne 2: Hero (Sequence-göreli 0–255) — ana foto + isim/kategori/kapasite ──
const Hero: React.FC<VillaReelProps> = (p) => {
  const frame = useCurrentFrame();
  ...
  const metaBits = [p.capacity, p.bedrooms, p.seaView].filter(Boolean);
  return (
    <AbsoluteFill style={{ opacity: outFade }}>
      ...
      <AbsoluteFill style={{ justifyContent: 'flex-end', padding: '0 70px 140px' }}>
        ...
        {metaBits.length ? (
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 14, marginTop: 24, opacity: oMeta }}>
            {metaBits.map((m, i) => (
              <span key={i} style={{ fontFamily: SANS, fontSize: 28, color: C.cream, fontWeight: 600, background: 'rgba(255,255,255,0.08)', border: `1px solid ${C.gold}55`, padding: '6px 16px', borderRadius: 999 }}>{m}</span>
            ))}
          </div>
        ) : null}
      </AbsoluteFill>
...
const Gallery: React.FC<VillaReelProps> = (p) => {
  const frame = useCurrentFrame();
  const { o } = useAppear(6);
  const extra = p.photos.slice(1, 4); // 2., 3., 4. fotolar
  // Villa havuzu küçük — tek fotolu villada galeri boş kalmasın diye hero foto'ya düş.
  const shots = extra.length ? extra : (p.photos[0] ? [p.photos[0]] : []);
  const per = 100;
  ...
  return (
    <AbsoluteFill style={{ opacity: outFade }}>
      ...
      <AbsoluteFill style={{ justifyContent: 'flex-end', padding: '0 70px 150px', pointerEvents: 'none' }}>
        ...
        <div style={{ fontFamily: SANS, fontSize: 26, letterSpacing: 5, color: C.gold, fontWeight: 800, opacity: o, position: 'relative' }}>VİLLADAN</div>
        {p.tagline ? (
...
// ── Sahne 4: Bilgi kartı (Sequence-göreli 0–180) — kapasite/havuz/manzara/konum ──
...
const Info: React.FC<VillaReelProps> = (p) => {
  const frame = useCurrentFrame();
  const { o } = useAppear(6);
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', padding: '0 80px', gap: 44, opacity: outFade }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 38, marginTop: 10 }}>
        <InfoRow icon="👥" label="KAPASİTE" value={p.capacity} from={20} />
        <InfoRow icon="🛏️" label="YATAK ODASI" value={p.bedrooms} from={32} />
        <InfoRow icon="🏊" label="HAVUZ" value={p.pool} from={44} />
        <InfoRow icon="🌊" label="MANZARA" value={p.seaView} from={56} />
        <InfoRow icon="📍" label="KONUM" value={p.location} from={68} />
      </div>
    </AbsoluteFill>
  );
};
...
const Outro: React.FC<VillaReelProps> = (p) => {
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', padding: 80 }}>
      ...
      <div style={{
        ...
        {p.cta || 'kalkaninfo.com/villalar'}
      </div>
    </AbsoluteFill>
  );
};

export const VillaReel: React.FC<VillaReelProps> = (props) => {
  return (
