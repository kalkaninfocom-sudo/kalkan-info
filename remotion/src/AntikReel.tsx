...
/**
 * AntikReel — "Haftanın Antik Kenti" tanıtım reel'i (RestoranReel ikizi, Çar slotu).
 * Canlı site CAPTURE ETMEZ → çerez/install banner bug'ı YOK. İçerik data/antik-kentler.json'dan
 * (build-antik-reel.mjs seçer) props ile gelir. 1080x1920, 30fps, 900 frame (30sn).
...
export const antikReelSchema = z.object({
  kicker: z.string().optional(),          // "KALKAN ÇEVRESİ"
  name: z.string(),
  category: z.string().optional(),        // "UNESCO Mirası"
  period: z.string().optional(),          // "Likya · Roma dönemi"
  entryFee: z.string().optional(),        // "600 ₺ / kişi (2026)"
  hours: z.string().optional(),           // ziyaret saatleri
  duration: z.string().optional(),        // "2-3 saat"
  distance: z.string().optional(),        // "15 km — batı"
  rating: z.number().optional(),
  tagline: z.string().optional(),         // az bilinen gerçek / editöryal satır
  photos: z.array(z.string()),            // tam URL (data:// veya https://)
  cta: z.string().optional(),             // kalkaninfo.com/...
});
export type AntikReelProps = z.infer<typeof antikReelSchema>;

export const defaultAntikReelProps: AntikReelProps = {
  kicker: 'KALKAN ÇEVRESİ',
  name: 'Patara Antik Kenti',
  category: 'UNESCO Mirası',
  period: 'Likya Birliği başkenti',
  entryFee: '600 ₺ / kişi (2026)',
  hours: '09:00 – 19:00 (Yaz)',
  duration: '2-3 saat',
  distance: '15 km — batı',
  rating: 5,
  tagline: "Dünyanın ilk demokratik meclisi Bouleuterion burada, M.Ö. 2. yüzyılda kuruldu.",
  photos: [],
  cta: 'kalkaninfo.com/antik-kentler.html',
};
...
const Intro: React.FC<AntikReelProps> = (p) => {
...
const Hero: React.FC<AntikReelProps> = (p) => {
  const frame = useCurrentFrame();
    ...
    <AbsoluteFill style={{ opacity: outFade }}>
      ...
      <AbsoluteFill style={{ justifyContent: 'flex-end', padding: '0 70px 140px' }}>
        <div style={{ display: 'inline-block', alignSelf: 'flex-start', background: C.gold, color: C.navy, fontFamily: SANS, fontWeight: 800, fontSize: 24, letterSpacing: 3, padding: '8px 18px', borderRadius: 4, opacity: o, marginBottom: 22 }}>
          {(p.category || 'ANTİK KENT').toUpperCase()}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 24, opacity: oMeta }}>
          ...
          {p.period ? <span style={{ fontFamily: SANS, fontSize: 30, color: C.cream, fontWeight: 600 }}>{p.period}</span> : null}
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
...
const Gallery: React.FC<AntikReelProps> = (p) => {
  const frame = useCurrentFrame();
  const { o } = useAppear(6);
  ...
  return (
    <AbsoluteFill style={{ opacity: outFade }}>
      ...
      <AbsoluteFill style={{ justifyContent: 'flex-end', padding: '0 70px 150px', pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: '40%', background: 'linear-gradient(180deg, rgba(4,20,35,0) 0%, rgba(4,20,35,0.75) 100%)' }} />
        <div style={{ fontFamily: SANS, fontSize: 26, letterSpacing: 5, color: C.gold, fontWeight: 800, opacity: o, position: 'relative' }}>BİLİYOR MUYDUNUZ?</div>
        {p.tagline ? (
          <div style={{ fontFamily: SERIF, fontWeight: 800, fontSize: 48, color: C.cream, lineHeight: 1.18, marginTop: 14, opacity: o, position: 'relative', textShadow: '0 4px 24px rgba(0,0,0,0.7)' }}>
            {p.tagline}
          </div>
        ) : null}
      </AbsoluteFill>
...
// ── Sahne 4: Bilgi kartı (Sequence-göreli 0–180) — dönem/giriş/saatler/mesafe/süre ──
...
const Info: React.FC<AntikReelProps> = (p) => {
  const frame = useCurrentFrame();
  const { o } = useAppear(6);
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', padding: '0 80px', gap: 44, opacity: outFade }}>
      <div style={{ fontFamily: SERIF, fontSize: 56, fontWeight: 900, color: C.cream, opacity: o, lineHeight: 1.05, letterSpacing: -1 }}>{p.name}</div>
      <div style={{ marginTop: -20 }}><Rule w={160} o={o} h={5} /></div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 36, marginTop: 10 }}>
        <InfoRow icon="🏛️" label="DÖNEM" value={p.period} from={20} />
        <InfoRow icon="🎟️" label="GİRİŞ" value={p.entryFee} from={32} />
        <InfoRow icon="🕐" label="ZİYARET SAATLERİ" value={p.hours} from={44} />
        <InfoRow icon="🚗" label="MESAFE" value={p.distance} from={56} />
        <InfoRow icon="⏱️" label="GEZİ SÜRESİ" value={p.duration} from={68} />
      </div>
    </AbsoluteFill>
  );
};
...
const Outro: React.FC<AntikReelProps> = (p) => {
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', padding: 80 }}>
      ...
      <div style={{
        marginTop: 50, background: C.gold, color: C.navy, fontFamily: SANS, fontWeight: 800, fontSize: 36,
        padding: '22px 44px', borderRadius: 12, opacity: o2, transform: `scale(${pulse})`, boxShadow: `0 16px 44px ${C.gold2}66`,
      }}>
        {p.cta || 'kalkaninfo.com/antik-kentler.html'}
      </div>
    </AbsoluteFill>
  );
};

export const AntikReel: React.FC<AntikReelProps> = (props) => {
  return (
