#!/usr/bin/env node
/**
 * scripts/agency/villa-reel-approval.mjs — HAFTANIN VİLLASI reel ONAY (💰 flagship).
 * ---------------------------------------------------------------------------------
 * restoran-reel-approval.mjs ikizi; villa reel'ine uyarlanmış. Akış:
 *   1. Reel render (build-villa-reel.mjs) → dist/social/villa/villa-reel.mp4
 ...
 *
 * Kullanım: node scripts/agency/villa-reel-approval.mjs [slug|isim]
 */
...
async function sendVideoUpload(mp4Path, caption, postId) {
  ...
  form.append('video', new Blob([buf], { type: 'video/mp4' }), 'villa-reel.mp4');
...
async function main() {
  console.log(`\n════ VILLA REEL ONAY — Haftanın Villası (${date}) ════`);

  // 1) Render
  console.log('── Reel render (build-villa-reel) ──');
  const args = ['scripts/agency/build-villa-reel.mjs'];
  if (forceArg) args.push(forceArg);
  const r = spawnSync('node', args, { cwd: ROOT, stdio: 'inherit' });
  const mp4 = join(ROOT, 'dist', 'social', 'villa', 'villa-reel.mp4');
  if (r.status !== 0 || !existsSync(mp4)) { console.error('❌ Reel render edilemedi:', mp4); process.exit(1); }
  console.log('✓ Reel hazır:', mp4);

  // Props'tan villa bilgisi (caption için)
  let p = {};
  try { p = JSON.parse(await readFile(join(ROOT, 'remotion', 'props-villa.json'), 'utf8')); } catch {}
  const name = p.name || 'Kalkan villası';
  const slug = (p.cta || '').split('/').filter(Boolean).pop() || '';

  if (!SUPA_URL || !SUPA_KEY) { console.warn('ℹ Supabase env yok — yükleme/onay atlandı. (Reel diskte.)'); return; }

  // 2) Storage'a yükle (villa bazlı benzersiz yol → tekrar üretimde üzerine yazar)
  const objectPath = `villa-reel/villa-reel-${date}${slug ? '-' + slug : ''}.mp4`;
  ...
  // 3) social_posts satırı (app-seviyesi upsert; haftada bir villa → packId tarihli)
  const packId = `villa-reel-${date}`;
  let post;
...
🇬🇧 Villa of the week in Kalkan — ${name}. Details & booking: ${cta}`;
  ...
  // Dile-özel sabit başlık + CTA etiketi (çeviriye gönderilmez → tutarlı); yalnız name/tagline çevrilir.
  const HDR = { en: 'VILLA OF THE WEEK', de: 'VILLA DER WOCHE', ru: 'ВИЛЛА НЕДЕЛИ', fr: 'VILLA DE LA SEMAINE' };
  const CTAL = { en: 'Details & booking', de: 'Details & Buchung', ru: 'Подробности и бронь', fr: 'Détails & réservation' };
  await publishReelTranslations({
    typeKey: 'villa', mp4Path: mp4, videoUrl, packIdBase: packId,
...
main().catch(e => { console.error('[villa-reel-approval]', e); process.exit(1); });
