#!/usr/bin/env node
/**
 * scripts/agency/restoran-reel-approval.mjs — HAFTANIN MEKÂNI restoran reel ONAY (💰 flagship).
 * -------------------------------------------------------------------------------------------
 * reel-approval.mjs (gazete) ikizi; restoran reel'ine uyarlanmış. Akış:
 *   1. Reel render (build-restoran-reel.mjs) → dist/social/restoran/restoran-reel.mp4
 ...
 *
 * Kullanım: node scripts/agency/restoran-reel-approval.mjs [slug|isim]
 */
...
async function sendVideoUpload(mp4Path, caption, postId) {
  ...
  form.append('video', new Blob([buf], { type: 'video/mp4' }), 'restoran-reel.mp4');
...
async function main() {
  console.log(`\n════ RESTORAN REEL ONAY — Haftanın Mekânı (${date}) ════`);

  // 1) Render
  console.log('── Reel render (build-restoran-reel) ──');
  const args = ['scripts/agency/build-restoran-reel.mjs'];
  if (forceArg) args.push(forceArg);
  const r = spawnSync('node', args, { cwd: ROOT, stdio: 'inherit' });
  const mp4 = join(ROOT, 'dist', 'social', 'restoran', 'restoran-reel.mp4');
  if (r.status !== 0 || !existsSync(mp4)) { console.error('❌ Reel render edilemedi:', mp4); process.exit(1); }
  console.log('✓ Reel hazır:', mp4);

  // Props'tan mekân bilgisi (caption için)
  let p = {};
  try { p = JSON.parse(await readFile(join(ROOT, 'remotion', 'props-restoran.json'), 'utf8')); } catch {}
  const name = p.name || 'Kalkan restoranı';
  const slug = (p.cta || '').split('/').pop() || '';

  if (!SUPA_URL || !SUPA_KEY) { console.warn('ℹ Supabase env yok — yükleme/onay atlandı. (Reel diskte.)'); return; }

  // 2) Storage'a yükle (mekân bazlı benzersiz yol → tekrar üretimde üzerine yazar)
  const objectPath = `restoran-reel/restoran-reel-${date}${slug ? '-' + slug : ''}.mp4`;
  ...
  // 3) social_posts satırı (app-seviyesi upsert; haftada bir mekân → packId tarihli)
  const packId = `restoran-reel-${date}`;
  let post;
...
🇬🇧 Restaurant of the week in Kalkan — ${name}. Details & menu: ${cta}`;
  ...
  const HDR = { en: 'RESTAURANT OF THE WEEK', de: 'RESTAURANT DER WOCHE', ru: 'РЕСТОРАН НЕДЕЛИ', fr: 'RESTAURANT DE LA SEMAINE' };
  const CTAL = { en: 'Details & menu', de: 'Details & Menü', ru: 'Подробности и меню', fr: 'Détails & menu' };
  await publishReelTranslations({
    typeKey: 'restoran', mp4Path: mp4, videoUrl, packIdBase: packId,
...
main().catch(e => { console.error('[restoran-reel-approval]', e); process.exit(1); });
