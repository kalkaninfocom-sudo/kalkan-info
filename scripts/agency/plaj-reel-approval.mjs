#!/usr/bin/env node
/**
 * scripts/agency/plaj-reel-approval.mjs — HAFTANIN PLAJI reel ONAY.
 * -----------------------------------------------------------------
 * restoran-reel-approval.mjs ikizi; plaj reel'ine uyarlanmış. Akış:
 *   1. Reel render (build-plaj-reel.mjs) → dist/social/plaj/plaj-reel.mp4
 ...
 *
 * Kullanım: node scripts/agency/plaj-reel-approval.mjs [slug|isim]
 */
...
async function sendVideoUpload(mp4Path, caption, postId) {
  ...
  form.append('video', new Blob([buf], { type: 'video/mp4' }), 'plaj-reel.mp4');
...
async function main() {
  console.log(`\n════ PLAJ REEL ONAY — Haftanın Plajı (${date}) ════`);

  // 1) Render
  console.log('── Reel render (build-plaj-reel) ──');
  const args = ['scripts/agency/build-plaj-reel.mjs'];
  if (forceArg) args.push(forceArg);
  const r = spawnSync('node', args, { cwd: ROOT, stdio: 'inherit' });
  const mp4 = join(ROOT, 'dist', 'social', 'plaj', 'plaj-reel.mp4');
  if (r.status !== 0 || !existsSync(mp4)) { console.error('❌ Reel render edilemedi:', mp4); process.exit(1); }
  console.log('✓ Reel hazır:', mp4);

  // Props'tan plaj bilgisi (caption için)
  let p = {};
  try { p = JSON.parse(await readFile(join(ROOT, 'remotion', 'props-plaj.json'), 'utf8')); } catch {}
  const name = p.name || 'Kalkan plajı';
  ...
  // 2) Storage'a yükle (plaj bazlı benzersiz yol → tekrar üretimde üzerine yazar)
  const objectPath = `plaj-reel/plaj-reel-${date}${slug ? '-' + slug : ''}.mp4`;
  ...
  // 3) social_posts satırı (app-seviyesi upsert; haftada bir plaj → packId tarihli)
  const packId = `plaj-reel-${date}`;
  let post;
