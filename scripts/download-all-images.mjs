 ...
 * Tüm dış görselleri (Wikimedia, Unsplash, picsum, placehold) /assets/img/<hash>.webp olarak indirir.
 * data/*.json ve *.html dosyalarındaki referansları tarar.
 * Output: assets/img/<hash>.webp + manifest.json güncellenir.
 */
import fs from 'fs';
...
async function processUrl(url, manifest) {
  ...
  if (manifest[url]) { process.stdout.write('.'); return { url, status: 'skip' }; }
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'KalkanInfoBot/1.0 (https://kalkaninfo.com; info@kalkan.info) Node/' + process.version,
...
async function main() {
  ...
  if (errors.length) {
    ...
    errors.forEach(e => console.log(`  ${e.url.slice(0, 80)}... → ${e.reason}`));
  }
