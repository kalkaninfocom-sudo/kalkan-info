// scripts/deniz-ops-call.mjs
// Deniz Ops → Berkay'ı arar ve günün raporunu okur (giden/outbound, Twilio üzerinden).
// Önce raporu tazeler (deniz-ops-report.mjs), sonra ElevenLabs outbound-call API'siyle arar.
//
// Çalıştırma:
//   node scripts/deniz-ops-call.mjs                 → BERKAY_PHONE env'ine arar
//   node scripts/deniz-ops-call.mjs +905XXXXXXXXX   → verilen numaraya arar
//   node scripts/deniz-ops-call.mjs --no-refresh     → raporu tazelemeden ara
//
...
(async () => {
  ...
  if (!noRefresh) {
    try { execFileSync('node', ['scripts/deniz-ops-report.mjs'], { cwd: ROOT, stdio: 'inherit' }); }
