// scripts/lyra-ops-report.mjs
// Günün ops raporunu üretir → sesli-özet'e çevirir (cheap-llm) → "Lyra Ops" ElevenLabs
// ajanının bilgi tabanına yazar. Berkay ajanı aradığında/mesaj attığında güncel raporu konuşur.
//
// Çalıştırma:
//   node scripts/lyra-ops-report.mjs           → üret + ajana yükle
//   node scripts/lyra-ops-report.mjs --dry-run → sadece özeti ekrana bas
//
...
(async () => {
  ...
  console.log('\n===== LYRA OPS — SESLİ RAPOR ÖZETİ =====\n');
  console.log(summary);
  console.log('\n=========================================\n');
  if (DRY) { console.log('(--dry-run: ajana yüklenmedi)'); return; }
  const docId = await pushToAgent(summary);
  console.log(`✅ Ops ajanına yüklendi (KB doc ${docId}). Lyra Ops artık güncel raporu konuşabilir.`);
})();
