#!/usr/bin/env node
/**
 * Kalkan/Kas cevresindeki 15 plaj icin Google Maps Photos (SerpApi) uzerinden gercek plaj fotograflarini ceker,
 * sharp ile yeniden boyutlandirir ve assets/img/plaj/ + assets/og/plaj/ altina kaydeder.
 *
 * Kaynak: serpapi engine=google_maps_photos (Google Maps'te halka acik fotograflar).
 * Maliyet: 1 cagri / plaj = ~$0.02. 15 plaj = ~$0.30 bir fetch.
 *
 * Kullanim:
 *   SERPAPI_KEY=... node scripts/fetch-plaj-photos.mjs
 *   SERPAPI_KEY=... node scripts/fetch-plaj-photos.mjs --force
 *   SERPAPI_KEY=... node scripts/fetch-plaj-photos.mjs --slug=kaputas,patara
 *
 * Cikti / plaj:
 *   assets/img/plaj/<slug>-hero.jpg     1920x1080 q82 mozjpeg
 *   assets/img/plaj/<slug>-1.jpg ... -8 1280x720 q82 mozjpeg
 *   assets/og/plaj/<slug>.jpg            1200x630 q82 mozjpeg (hero cropped)
 *   data/plaj-photos/<slug>.json         manifest (7 gun TTL)
 */
...
const PHOTOS_CACHE_DIR = join(ROOT, 'data', 'plaj-photos');
const REVIEWS_DIR = join(ROOT, 'data', 'plaj-reviews');
const IMG_DIR = join(ROOT, 'assets', 'img', 'plaj');
const OG_DIR = join(ROOT, 'assets', 'og', 'plaj');
...
async function fetchGoogleImages(name, region) {
  const r = region || 'Kalkan';
  const q = encodeURIComponent(`${name} ${r} plaj beach`);
...
for (const r of items) {
  ...
  if (placeId) {
    ...
    console.warn(`    place_id yok, dogrudan google_images fallback`);
  ...
  if (photoUrls.length < 4) {
    try {
      ...
      const extra = await fetchGoogleImages(r.name, r.region);
      serpApiCalls++;
      const merged = [...photoUrls, ...extra].filter((u, i, a) => a.indexOf(u) === i);
      photoUrls = merged;
      source = photoUrls.length > 0 && placeId ? `${source}+google_images` : 'google_images';
    } catch (e) {
  ...
  const manifest = {
    ...
    files: {
      hero: `/assets/img/plaj/${slug}-hero.jpg`,
      og: `/assets/og/plaj/${slug}.jpg`,
      gallery: galleryFiles.map(f => `/assets/img/plaj/${f}`),
    },
    source_urls: top.slice(0, 9),
    total_bytes: bytesThisPlaj,
  };
  await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
  fetched++;

  console.log(`  + ${slug}: ${galleryFiles.length + 1} foto + OG (${fmtSize(bytesThisPlaj)}) [${source}]`);
}

console.log(`\n=== Photo Fetch Raporu ===`);
console.log(`Fetched: ${fetched} plaj`);
console.log(`Cache hit: ${cached} plaj`);
