#!/usr/bin/env node
/**
 * Kalkan'in 15 oteli icin Google Maps Photos (SerpApi) uzerinden gercek otel fotograflarini ceker,
 * sharp ile yeniden boyutlandirir ve assets/img/oteller/ + assets/og/otel/ altina kaydeder.
 *
 * Kaynak: serpapi engine=google_maps_photos (Google Maps'te halka acik fotograflar).
 * Maliyet: 1 cagri / otel = ~$0.02. 15 otel = ~$0.30 bir fetch.
 *
 * Kullanim:
 *   SERPAPI_KEY=... node scripts/fetch-otel-photos.mjs
 *   SERPAPI_KEY=... node scripts/fetch-otel-photos.mjs --force
 *   SERPAPI_KEY=... node scripts/fetch-otel-photos.mjs --slug=white-house-kalkan,patara-prince-resort
 *
 * Cikti / otel:
 *   assets/img/oteller/<slug>-hero.jpg     1920x1080 q82 mozjpeg
 *   assets/img/oteller/<slug>-1.jpg ... -8 1280x720 q82 mozjpeg
 *   assets/og/otel/<slug>.jpg              1200x630 q82 mozjpeg (hero cropped)
 *   data/otel-photos/<slug>.json           manifest (7 gun TTL)
 */
...
// Fallback: google_images sorgusu (otel adi + Kalkan)
async function fetchGoogleImages(name) {
  const q = encodeURIComponent(`${name} otel Kalkan`);
...
function maximizeGoogleUrl(url) {
  if (!url) return url;
  // Genellikle ...=w408-h272-k-no veya =s1600 gibi suffix
...
// Cache'den otel place_id/data_id oku
...
for (const r of items) {
  ...
  // Cache check: manifest fresh ve hero dosyasi mevcut ise atla
  ...
  if (!placeId) {
    failed.push({ slug, reason: 'place_id/data_id yok (data/otel-reviews/...)' });
  ...
  try {
    ...
    serpApiCalls++;
    // SerpApi google_maps_photos -> { photos: [{ image, thumbnail, ... }] }
  ...
  // En iyi 9 fotograf: hero + 8 galeri
  const top = photoUrls.slice(0, 12); // buffer for failures
  let downloaded = [];
  ...
  // Galeri 1280x720 x 8 (hero hariç tum gecerli bufferlardan)
  ...
  let bytesThisHotel = 0;
  for (const f of fileList) {
    try {
      const s = await stat(join(IMG_DIR, f));
      bytesThisHotel += s.size;
      totalBytes += s.size;
      totalFiles++;
    } catch {}
  }
  ...
  const manifest = {
    ...
    files: {
      hero: `/assets/img/oteller/${slug}-hero.jpg`,
      og: `/assets/og/otel/${slug}.jpg`,
      gallery: galleryFiles.map(f => `/assets/img/oteller/${f}`),
    },
    source_urls: top.slice(0, 9),
    total_bytes: bytesThisHotel,
  };
  await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
  fetched++;

  console.log(`  + ${slug}: ${galleryFiles.length + 1} foto + OG (${fmtSize(bytesThisHotel)}) [${source}]`);
}

console.log(`\n=== Photo Fetch Raporu ===`);
console.log(`Fetched: ${fetched} otel`);
console.log(`Cache hit: ${cached} otel`);
