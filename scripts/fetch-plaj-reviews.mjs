#!/usr/bin/env node
/**
 * SerpApi uzerinden Kalkan/Kas cevresindeki 15 plaj icin Google Maps verisi + yorumlari ceker.
 * Cache: data/plaj-reviews/<slug>.json (7 gun TTL)
 *
 * Kullanim:
 *   SERPAPI_KEY=... node scripts/fetch-plaj-reviews.mjs              # cache > 7 gun ise fetch
 *   SERPAPI_KEY=... node scripts/fetch-plaj-reviews.mjs --force      # tum cache'i bypass et
 *   SERPAPI_KEY=... node scripts/fetch-plaj-reviews.mjs --slug=kaputas,patara
 *
 * Maliyet: 1 search + 1 reviews = ~$0.04/plaj. 15 plaj = ~$0.60 bir fetch.
 */
...
const CACHE_DIR = join(ROOT, 'data', 'plaj-reviews');
...
async function searchLocal(name, region) {
  const r = region || 'Kalkan';
  const q = encodeURIComponent(`${name} ${r} Antalya`);
...
for (const r of items) {
  ...
  try {
    console.log(`  ~ ${r.name}`);
    const local = await searchLocal(r.name, r.region);
