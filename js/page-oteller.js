(async () => {
  const data = await KalkanData.load('oteller');
  ...
  function render() {
    ...
    grid.innerHTML = items.map(KalkanData.otelCard).join('')
  ...
  searchEl.addEventListener('input', () => {
    ...
    searchDebounce = setTimeout(() => {
      ...
      if (q.length >= 2 && window.plausibleEvent) {
        window.plausibleEvent('search', { page: 'oteller', query_len: String(q.length) });
      }
    }, 600);
  });
  catEl.addEventListener('change', () => {
    ...
    if (window.plausibleEvent && catEl.value) {
      window.plausibleEvent('category_filter', { page: 'oteller', category: catEl.value });
    }
  });
  render();
})();
