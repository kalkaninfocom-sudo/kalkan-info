    ...
    return `
      <article class="card" style="background:white;border-radius:14px;overflow:hidden;box-shadow:0 1px 3px rgba(7,33,54,0.08);border:1px solid rgba(26,94,147,0.06);">
        ...
        <div class="p-4">
          ...
          <div class="text-xs text-ink-700/60 mt-1">${escape(t.duration||'')} · ${escape(t.capacity||'')}</div>
          ...
          <div class="flex items-center justify-between mt-4 pt-3 border-t border-ink-700/8">
            <div>
              <div class="font-bold text-sea-600">${escape(t.price||'')}</div>
              <div class="text-[11px] text-ink-700/60">${escape(t.priceNote||'')}</div>
            </div>
            <button class="bg-sun-500 text-ink-900 text-xs font-bold px-3 py-2 rounded-lg hover:bg-sun-400 transition">Rezerve Et</button>
          </div>
        </div>
      </article>
    `;
