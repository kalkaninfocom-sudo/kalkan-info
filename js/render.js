  ...
  function restoranCard(r) {
    ...
    return `
      <article class="card" style="background:white;border-radius:14px;overflow:hidden;box-shadow:0 1px 3px rgba(7,33,54,0.08);border:1px solid rgba(26,94,147,0.06);">
        ...
        <div class="p-4">
          <h3 class="font-display font-extrabold text-ink-900 text-lg leading-tight"><a ${nameAttrs} href="${r.customSiteUrl ? escape(r.customSiteUrl) : (r.detailPath || `restoran/${escape(r.id)}/`)}"${r.customSiteUrl ? ' target="_blank" rel="noopener"' : ''} class="hover:text-sea-700 transition-colors" style="color:inherit;text-decoration:none;">${escape(name)}</a></h3>
          ...
          <div class="flex items-center justify-between mt-4 pt-3 border-t border-ink-700/8 gap-2">
            <a href="${r.customSiteUrl ? escape(r.customSiteUrl) : (r.detailPath || `restoran/${escape(r.id)}/`)+'#reviews'}"${r.customSiteUrl ? ' target="_blank" rel="noopener"' : ''} onclick="event.stopPropagation();" class="inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-md transition" style="background:#f1f3f4;color:#1a73e8;" onmouseover="this.style.background='#e8eaed';" onmouseout="this.style.background='#f1f3f4';" title="${escape(uiLabel('googleReviewsTitle'))}" aria-label="${escape(name||'')} ${escape(uiLabel('googleReviews'))}">
