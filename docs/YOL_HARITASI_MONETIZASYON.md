...
**A) Satış motoru sahaya taşındı — PR #22 açık, MERGE bekliyor:**
...
- ⏳ **PR #22 (`sales-tools`→`main`) merge = hatırlatıcı cron canlı.** Secret'lar mevcut. Merge Berkay'da (main'e doğrudan push harness'ta engelli → PR yolu).

**B) Demo site şablonu BAŞTAN yazıldı (`scripts/agency/build-venue-site.mjs`) — Berkay BEĞENDİ 🟢, ama YERELDE (canlıya alınmadı, commit→PR bekliyor):**
...
- **(B)** venue-site redesign'ı commit→PR ile canlıya al (22 demo + `build-venue-site.mjs` + `venue-hero.json`).
