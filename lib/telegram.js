...
// file_id → indirilebilir mutlak URL (Telegram getFile)
export async function getFileUrl(fileId) {
  const r = await call('getFile', { file_id: fileId });
  return `${API_BASE}/file/bot${token()}/${r.file_path}`;
}

// Buffer'ı foto olarak gönder (multipart) — shop'lanan görsel geri dönüşü için.
// caption düz metin (parse_mode YOK — kaçış derdi olmasın).
export async function sendPhotoBuffer(chatId, buffer, caption) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
  if (caption) form.append('caption', String(caption));
  form.append('photo', new Blob([buffer], { type: 'image/png' }), 'shopped.png');
  const res = await fetch(`${API_BASE}/bot${token()}/sendPhoto`, { method: 'POST', body: form });
  const json = await res.json().catch(() => ({}));
  if (!res.ok || !json.ok) throw new Error(`telegram sendPhoto fail: ${json.description || res.status}`);
  return json.result;
}
