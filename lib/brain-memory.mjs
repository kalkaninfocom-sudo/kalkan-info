...
/** Uzak (Supabase) okuma — REMOTE modda hafızayı buluttan çeker. Başarısızsa []. */
async function recallRemote({ kind, since, limit } = {}) {
  if (!REMOTE) return [];
  try {
    let url = `${SUPA_URL}/rest/v1/brain_memory?select=id,ts,kind,tags,data&order=ts.desc`;
    if (kind) url += `&kind=eq.${encodeURIComponent(kind)}`;
    if (since) url += `&ts=gte.${encodeURIComponent(new Date(since).toISOString())}`;
    url += `&limit=${limit && limit < 2000 ? limit : 2000}`;
...
/**
 * REMOTE-AWARE sorgu — Actions gibi ephemeral ortamlar için ŞART.
 * REMOTE modda Supabase + yerel JSONL'i id'ye göre birleştirir; değilse sadece yerel.
 * query() ile aynı filtreleri destekler ama ASYNC. Cold-start'ın kök nedenini (yerel dosya
 * her CI run'ında boş) çözer.
 */
export async function recall({ kind, since, tag, filter, limit } = {}) {
  const sinceMs = since ? new Date(since).getTime() : null;
  let rows = readAll();
  if (REMOTE) {
    const remote = await recallRemote({ kind, since, limit });
    const seen = new Set(rows.map(r => r.id));
    for (const r of remote) if (!seen.has(r.id)) { rows.push(r); seen.add(r.id); }
  }
  if (kind) rows = rows.filter(r => r.kind === kind);
  if (tag) rows = rows.filter(r => Array.isArray(r.tags) && r.tags.includes(tag));
  if (sinceMs != null) rows = rows.filter(r => new Date(r.ts).getTime() >= sinceMs);
  if (typeof filter === 'function') rows = rows.filter(filter);
  rows.sort((a, b) => new Date(b.ts) - new Date(a.ts)); // yeni→eski
  return limit ? rows.slice(0, limit) : rows;
}
