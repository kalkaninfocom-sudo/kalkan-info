 ...
 */
export function dedupeNews(items, { threshold = 0.6, scoreFn = null, titleKey = 'title' } = {}) {
  ...
  for (const it of ordered) {
    ...
    // Kısa başlıklar (≤3 token) için dedup atla: "Kaş plaj uyarısı" vs "Kaş plaj kapandı"
    // gibi farklı haberlerin yanlışlıkla çökmemesi için daha yüksek benzerlik şart.
    if (sig.size && sig.size > 3 && sigs.some(s => s.size > 3 && jaccard(sig, s) >= threshold)) continue; // aynı hikaye — atla
