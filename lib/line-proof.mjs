...
export const fmtAge = (d) => (d === Infinity ? '?' : d < 1 ? `${Math.round(d * 24)}s önce` : `${d.toFixed(1)}g önce`);
...
export function freshnessCheck({ root, name, file, fields, maxAgeDays, critical = true }) {
  ...
  if (age === Infinity) return { name, ok: false, critical: false, detail: `tarih okunamadı (${file})` };
