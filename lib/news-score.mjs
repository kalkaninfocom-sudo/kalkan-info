 ...
 *   boost         {Map<string,number>|null}  OPSİYONEL — 2. beyin engagement sinyali.
 *                 Anahtar: küçük-harf anahtar kelime/konu. Değer: önerilen bonus (0..3 arası).
 *                 Her eşleşen anahtar kelime için değeri eklenir, toplam [0,3] ile sınırlanır.
 *                 Sağlanmazsa (undefined/null) davranış HİÇ DEĞİŞMEZ — geriye uyumlu.
 */
export function newsScore(it, { useTags = true, featuredBonus = true, srcPenalty = -5, boost = null } = {}) {
  ...
  // 2. BEYİN ENGAGEMENT BOOST (opsiyonel, sınırlı): en fazla +3, asla dominant olamaz.
  // boost sağlanmadıysa (null/undefined) bu blok çalışmaz → eski davranış korunur.
  if (boost instanceof Map && boost.size > 0) {
    const txtLow = txt.toLocaleLowerCase('tr');
    let engBonus = 0;
    for (const [kw, val] of boost) {
      if (txtLow.includes(kw)) engBonus += val;
    }
    s += Math.min(3, Math.max(0, engBonus)); // [0,3] ile sınırla
  }
  return s;
}
