...
// IG business ID'yi normalize et — env/secret'a yapışabilen trailing 'n' (BigInt kopyası),
// boşluk veya tırnakları temizler. 38 yayın '...523227n' hatasıyla düştü; bu onu kalıcı önler.
function normIgId(id) {
  const clean = String(id ?? '').replace(/[^0-9]/g, '');
  if (!clean) throw new Error(`Geçersiz IG business ID: "${id}"`);
  return clean;
}
