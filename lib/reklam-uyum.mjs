...
// ── Madde 1 / AB AI Act Madde 50: Yapay zeka kullanımı açıkça + makine-okunur belirtilmeli ────
// AB AI Act Madde 50 (yür. 2 Ağustos 2026, uyum 2 Aralık 2026): kamuyu bilgilendirme amaçlı
// yayınlanan AI-üretimi metin (gazete = tam kapsam) hem GÖRÜNÜR etiket hem MAKİNE-OKUNUR
// işaret taşımalı. 5 dil (TR/EN/DE/RU/FR) — gazetenin ürettiği tüm diller.
...
export const AI_IBARESI_DE = '🤖 Dieser Inhalt wurde mithilfe von künstlicher Intelligenz erstellt.';
export const AI_IBARESI_RU = '🤖 Этот материал создан с помощью искусственного интеллекта.';
export const AI_IBARESI_FR = '🤖 Ce contenu a été créé avec l’aide de l’intelligence artificielle.';
export const AI_IBARESI = { tr: AI_IBARESI_TR, en: AI_IBARESI_EN, de: AI_IBARESI_DE, ru: AI_IBARESI_RU, fr: AI_IBARESI_FR };
export const AI_HASHTAGS = ['#yapayzeka'];

const HAS_AI_MENTION = /yapay\s*zek[âa]|#yapayzeka|\byapayzeka\b|\bA\.?I\.?\b|üretken zeka|künstliche|intelligence|интеллект/i;

/** İbareyi dile göre getir (bilinmeyen dil → TR). */
export function aiIbaresi(lang = 'tr') {
  return AI_IBARESI[String(lang || 'tr').toLowerCase()] || AI_IBARESI_TR;
}

/**
 * AI ile üretilmiş içeriğin caption'ına şeffaflık ibaresini + #yapayzeka ekler (yoksa). 5 dil.
 * @param {string} caption
 * @param {{hashtags?:string[], lang?:'tr'|'en'|'de'|'ru'|'fr', isAI?:boolean}} opts
...
/**
 * AB AI Act Madde 50 MAKİNE-OKUNUR işaret + görünür footer — gazete HTML sayfalarına gömülür.
 * <head> içine meta, sayfa altına görünür ibare. Statik HTML render'ında build.mjs kullanır.
 * @param {{lang?:string, model?:string}} opts
 * @returns {{metaTags:string, footerHtml:string, footerText:string}}
 */
export function aiContentMeta({ lang = 'tr', model = 'kalkan-newsroom' } = {}) {
  const line = aiIbaresi(lang);
  // Makine-okunur: hem klasik meta hem schema.org disambiguatingDescription uyumlu attribute.
  const metaTags = [
    '<meta name="ai-generated" content="true">',
    '<meta name="generator" content="AI-assisted (' + model + ')">',
    '<meta name="robots-ai-disclosure" content="ai-assisted-content">',
  ].join('\n');
  const footerHtml = '<p class="ai-disclosure" data-ai-generated="true" role="note">' + line + '</p>';
  return { metaTags, footerHtml, footerText: line };
}
...
export default { withAiDisclosure, aiIbaresi, aiContentMeta, hedefliReklamNotu, scanReklamUyum, AI_IBARESI, AI_IBARESI_TR, AI_IBARESI_EN, AI_IBARESI_DE, AI_IBARESI_RU, AI_IBARESI_FR, AI_HASHTAGS };
