...
const PROVIDERS = {
  ...
  // Cerebras — ücretsiz, KART YOK, 1M token/gün, çok hızlı (Llama 3.3 70B / Qwen3 / GPT-OSS-120B).
  cerebras: {
...
function order() {
  ...
  return ['ollama', 'cerebras', 'groq', 'nvidia', 'gemini', 'claude'];
}
...
export async function cheapLLM(prompt, opts = {}) {
  ...
  const seq = opts.only ? [opts.only] : order();
  const errors = [];
